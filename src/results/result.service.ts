...
function normalizeStatus(value: unknown, marks: number, maxMarks: number): ResultStatus {
  if (value === 'PASS' || value === 'FAIL' || value === 'WITHHELD') return value;
...
function toResultResponse(result: any) {
...
function summaryFromResults(results: any[]) {
...
export async function listResults(
  ...
  if (query.examType) where.exam_type = query.examType;
  if (query.semester !== undefined) where.semester = query.semester;
  if (query.academicYear) where.academic_year = query.academicYear;
...
export async function saveResult(
  ...
  const existing = input.id
    ? await prisma.studentResult.findFirst({
